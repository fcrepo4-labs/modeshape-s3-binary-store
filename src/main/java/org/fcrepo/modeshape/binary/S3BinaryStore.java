 ...
 */
package org.fcrepo.modeshape.binary;
...
/**
 * Custom binary storage option for ModeShape which manages the storage
...
public class S3BinaryStore extends AbstractBinaryStore {
     ...
     *
     * In order to connect to AWS, an access key ID and secret key must be made available
    ...
    @Override
    public void removeValuesUnusedLongerThan(long minimumAge, TimeUnit timeUnit)
        throws BinaryStoreException {
        ...
        try {
            for(BinaryKey key : getAllBinaryKeys()) {
                ObjectMetadata metadata =
                    s3Client.getObjectMetadata(bucketName, key.toString());
